...
export default function EditMembership() {
    ...
    const fetchMembership = async () => {
        try {
            const memberships = await api.admin.memberships.list();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            await api.admin.memberships.update(id!, formData);
            navigate("/memberships");
        } catch (err: any) {
    ...
    const removeType = (index: number) => {
        setFormData({
            ...formData,
            membership_types: formData.membership_types.filter((_: any, i: number) => i !== index)
        });
    };

    if (loading) return <div className="flex justify-center p-12 text-gray-500 font-bold">Loading membership...</div>;
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-black tracking-tight">Edit Membership</h1>
                    <p className="text-gray-400 font-medium mt-1">Modify membership tiers and custom application fields.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 pb-20">
                ...
                <div className="bg-white rounded-[1.5rem] border border-gray-100 p-8 shadow-sm space-y-6">
                    ...
                    <div className="space-y-4">
                        {formData.membership_types.map((type: any, i: number) => (
                ...
                <div className="bg-white rounded-[1.5rem] border border-gray-100 p-8 shadow-sm space-y-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {formData.attended_programs_options.map((opt: string, i: number) => (
                                <div key={i} className="flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-xl border border-gray-100 text-sm font-bold text-black group">
                                    ...
                                    <button 
                                        ...
                                        onClick={() => setFormData({
                                            ...formData, 
                                            attended_programs_options: formData.attended_programs_options.filter((_: any, idx: number) => idx !== i)
                                        })}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {formData.interest_options.map((opt: string, i: number) => (
                                <div key={i} className="flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-xl border border-gray-100 text-sm font-bold text-black group">
                                    {opt}
                                    <button 
                                        type="button"
                                        onClick={() => setFormData({
                                            ...formData, 
                                            interest_options: formData.interest_options.filter((_: any, idx: number) => idx !== i)
                                        })}
                                        className="text-gray-300 hover:text-red-500 transition-colors"
                                    >
                                        &times;
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {formData.custom_questions.map((q: any, i: number) => (
                                <div key={i} className="flex gap-4 items-center bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                    ...
                                    <button 
                                        ...
                                        onClick={() => setFormData({
                                            ...formData,
                                            custom_questions: formData.custom_questions.filter((_: any, idx: number) => idx !== i)
                                        })}
                ...
                <div className="flex justify-end gap-4">
                    ...
                    >
                        {submitting ? "Updating..." : "Update Membership"}
